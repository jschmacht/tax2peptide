...
class TestCustomTaxonDB(unittest.TestCase):
    ...
    def test_emptyTaxIDs(self):
        taxIDs=set()
        write_db = WriteCustomDB("./testdata/example_database_uniprot.fasta", "./testdata/customized_uniprot.fasta", taxIDs)
        write_db.read_database(True, gzipped=False, threads=3)
        file = open("./testdata/customized_uniprot.fasta", "r")
        self.assertEqual(file.read(), "")
        file.close()

    def test_emptyDatabase(self):
        taxIDs = {5, 6, 26}
        write_db = WriteCustomDB("./testdata/empty_database.fasta", "./testdata/customized_uniprot.fasta", taxIDs)
        write_db.read_database(True, gzipped=False, threads=3)
        file = open("./testdata/customized_uniprot.fasta", "r")
        self.assertEqual(file.read(), "")
        file.close()
    ...
    def test_ncbi_db_search(self):
        ...
        write_db = WriteCustomDB("./testdata/example_database_ncbi.fasta", "./testdata/customized_ncbi.fasta")
        write_db.read_database(with_taxon_ID=False, gzipped=False, threads=3, accessions=accessionIDs)
        test_database = open("./testdata/result_test_ncbi.fasta", "r")
        td = test_database.read()
        test_database.close()
        written_database = open("./testdata/customized_ncbi.fasta", "r")
    ...
    def test_ncbi_db_search_gzipped(self):
        accessionIDs = {'NP_268346.1', 'NP_5.2', 'NP_3.2', 'AAN62213.1', 'NP_9.2', 'pl_12'}
        write_db = WriteCustomDB("./testdata/example_database_ncbi.fasta.gz", "./testdata/customized_ncbi.fasta")
    ...
    def test_uniprot_db_search(self):
        ...
        test_database = open("./testdata/result_test_uniprot.fasta", "r")
        td = test_database.read()
        test_database.close()
        written_database = open("./testdata/customized_uniprot.fasta", "r")
        wd = written_database.read()
        written_database.close()
        self.assertEqual(wd.count('>'), td.count('>'))
        self.assertEqual(wd.count('\n'), td.count('\n'))
    ...
    def test_uniprot_db_search_gzipped(self):
        taxIDs = {6, 7, 34, 26}
        write_db = WriteCustomDB("./testdata/example_database_uniprot.fasta.gz", "./testdata/customized_uniprot.fasta", taxIDs)
